...
import com.gym.mapper.annotation.MemberMapper;               // 🗺 어노테이션 매퍼
...
public class UserMypageController {
    ...
    // 💉 매퍼 주입
    private final MemberMapper memberMapper;      // 🔄 DB 호출 매퍼

    /* ============================================================
    [마이페이지] 회원 단건 조회(배선 점검용)
    - 목적: member_tbl 단건 조회 → ApiResponse 포맷으로 반환
    - 주의: 실전 전 인증/권한(@PreAuthorize) 및 Service 레이어로 분리 예정
    ============================================================ */
    @Operation(summary = "회원 단건 조회", description = "memberId로 member_tbl에서 회원 정보를 조회합니다.")
    ...
    public ApiResponse<Member> getMemberById(
        ...
        Member member = memberMapper.selectMemberById(memberId);     // ✅ DB 조회
